...
mod memory_safety_tests {
    ...
    fn test_null_string_safety() {
        ...
        // null ポインタでの文字列長取得
        let len = unsafe { yuni_str_len(std::ptr::null()) };
        assert_eq!(len, 0, "null ポインタの文字列長は0である必要があります");
        
        // null ポインタでの文字列結合
        let result = unsafe { yuni_string_concat(std::ptr::null(), std::ptr::null()) };
        assert!(result.is_null(), "null ポインタの結合はnullを返す必要があります");
        
        let valid_str = CString::new("test").unwrap();
        let result = unsafe { yuni_string_concat(valid_str.as_ptr(), std::ptr::null()) };
        assert!(result.is_null(), "片方がnullの場合はnullを返す必要があります");
        
        let result = unsafe { yuni_string_concat(std::ptr::null(), valid_str.as_ptr()) };
        assert!(result.is_null(), "片方がnullの場合はnullを返す必要があります");
    }
    ...
    fn test_string_memory_management() {
        ...
        let result_ptr = unsafe { yuni_string_concat(s1.as_ptr(), s2.as_ptr()) };
    ...
    fn test_multiple_string_operations() {
        ...
        // 段階的な文字列結合
        let temp1 = unsafe { yuni_string_concat(s1.as_ptr(), s2.as_ptr()) };
        assert!(!temp1.is_null());
        
        let temp2 = unsafe { yuni_string_concat(temp1, s3.as_ptr()) };
        assert!(!temp2.is_null());
        
        let final_result = unsafe { yuni_string_concat(temp2, s4.as_ptr()) };
